...
describe command "#{openrc} openstack recordset create --record '10.0.0.1' --type A example.com. www && sleep 5\"" do
  its('exit_status') { should eq 0 }
end
...
describe command 'dig @localhost +short SOA example.com.' do
  its('exit_status') { should eq 0 }
  its('stdout') { should match /ns1.example.org. dnsmaster.example.com./ }
end

describe command 'dig @localhost +short www.example.com.' do
  its('exit_status') { should eq 0 }
  its('stdout') { should match /^10.0.0.1$/ }
end

describe command "#{openrc} openstack zone delete example.com. && sleep 5\"" do
  its('exit_status') { should eq 0 }
end

describe command "#{openrc} openstack zone list -f value -c name -c type -c status -c action\"" do
  its('exit_status') { should eq 0 }
  its('stdout') { should_not match /^example.com. PRIMARY ACTIVE NONE$/ }
end

describe command 'dig @localhost +short SOA example.com.' do
  its('exit_status') { should eq 0 }
  its('stdout') { should_not match /ns1.example.org. dnsmaster.example.com./ }
end

describe command 'dig @localhost +short www.example.com.' do
  its('exit_status') { should eq 0 }
  its('stdout') { should_not match /^10.0.0.1$/ }
end
